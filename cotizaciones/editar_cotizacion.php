...
<script>
    $(document).ready(function() {
                ...
                productos.forEach((item, i) => {
                    ...
                    $bodyP.append(`
                        <tr>
                          ...
                          <td>
                            <input type="number" class="form-control form-control-sm precioUnitarioInput"
                                   data-index="${idx}"
                                   value="${item.precioUnitario.toFixed(2)}"
                                   min="0" step="0.01">
                          </td>
                          <td class="subtotal-item">S/ ${sub.toFixed(2)}</td>
        ...
        function updateCalculationsAndDisplay() {
            ...
            if (productos.length) {
                $detalleDiv.find('.total-neto-productos').text(`S/ ${totalProdNet.toFixed(2)}`); // Agregado para actualizar el neto
            ...
            if (servicios.length) {
                $detalleDiv.find('.total-neto-servicios').text(`S/ ${totalServNet.toFixed(2)}`); // Agregado para actualizar el neto
        ...
        $('#detalleCotizacion')
            ...
            // NUEVO: Evento para cambiar el precio unitario
            .on('input', '.precioUnitarioInput', function() {
                const idx = +$(this).data('index');
                let nuevoPrecio = parseFloat($(this).val());
                if (isNaN(nuevoPrecio) || nuevoPrecio < 0) {
                    nuevoPrecio = 0; // O manejar como desees para valores inválidos
                }
                itemsCotizacion[idx].precioUnitario = nuevoPrecio;

                // Actualizar el subtotal del item específico en la tabla
                const cantidad = itemsCotizacion[idx].cantidad;
                $(this).closest('tr').find('.subtotal-item').text(`S/ ${(cantidad * nuevoPrecio).toFixed(2)}`);
        ...
        // ── PREVIEW y GUARDAR ──
          $('#btnPreview').on('click', function(e) {
              ...
              $.post('preview_cotizacion.php', data, html=>{
                  ...
                  $('#detalleCotizacion, .form-group, #crearCotizacionForm button').hide(); // Oculta todos los form-groups y botones
                  $('#volverEditar').show();
        ...
        $('#volverEditar').on('click', ()=> {
            ...
            $('.form-group, #crearCotizacionForm button').show(); // Muestra todos los form-groups y botones
            $('#detalleCotizacion').show();
